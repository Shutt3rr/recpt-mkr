...
def write_receipt(item_prices, total_sales_tax, total_amount):
    with open("receipt.txt", "w") as f:
        ...
        for index, price in enumerate(item_prices, start=1):
            print(f"Item {index}: ${price:.2f}", file=f)
    
        print("-" * 30, file=f)
        print(f"Sales Tax ({round(total_sales_tax * 100, 2)}%): ${total_sales_tax:.2f}", file=f)
        print("=" * 30, file=f)
        print(f"Total Amount: ${total_amount:.2f}", file=f)
        print("=" * 30, file=f)
...
def main():
    ...
    if item_prices:
        ...
        write_receipt(item_prices, total_sales_tax, total_amount)
    else:
